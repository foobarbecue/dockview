...
You can define an `initialWidth` and `initialHeight`. The dock will make a best attempt to obey these inputs but it may not always be possible due to the constraints of the grid.
